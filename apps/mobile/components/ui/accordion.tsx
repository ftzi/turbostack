...
function Accordion({
	children,
	...props
}: Omit<AccordionPrimitive.RootProps, "asChild"> & React.RefAttributes<AccordionPrimitive.RootRef>) {
	return (
		<LayoutAnimationConfig skipEntering>
			<AccordionPrimitive.Root {...(props as AccordionPrimitive.RootProps)} asChild={Platform.OS !== "web"}>
...
function AccordionItem({
	...
	return (
		<AccordionPrimitive.Item
			className={cn("border-border border-b", Platform.select({ web: "last:border-b-0" }), className)}
			value={value}
	...
	return (
		<TextClassContext.Provider value="text-sm">
			<AccordionPrimitive.Content
				...
				{...props}
			>
				<Animated.View exiting={Platform.select({ native: FadeOutUp.duration(200) })} className={cn("pb-4", className)}>
					{children}
				</Animated.View>
			</AccordionPrimitive.Content>
		</TextClassContext.Provider>
